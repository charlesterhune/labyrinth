...
export const usePlayersStore = defineStore("players", {
        ...
        findByMazeCard() {
            return function (cardId) {
                return this.all.filter(
                    (player) => player.mazeCardId === cardId
                );
            };
        },
    ...
    actions: {
        update(apiPlayers) {
            const apiIds = apiPlayers.map(
                (apiPlayer) => apiPlayer.id
            );

            const removedPlayerIds =
                this.allIds.filter(
                    (id) => !apiIds.includes(id)
                );
            ...
            // Re-attach Wix info to the local user
            this.applyWixUserInfo();
        },
            ...
            if (gameStore.isOnline) {
                API.doAddPlayer(
                    ...
                    (apiPlayer) => {
                        ...
                        if (this.userPlayerName) {
                            apiPlayer.name =
                                this.userPlayerName;
                        }

                        if (this.userProfilePic) {
                            apiPlayer.profilePic =
                                this.userProfilePic;
                        }
